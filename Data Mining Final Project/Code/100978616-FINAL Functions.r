...
find_unique_variables <- function(data) {
  res <- NULL
...
find_best_seedv2 <- function(data, method, min_seed, max_seed, min_k, max_k, real_class) {
  ...
  for (k in min_k:max_k) {
    print(paste0("Results for k=",k,type_data))
    print("=======================")
...
# Function to Saving (Convert) the Code from the Report (\Report\FINAL-PROJECT.Rmd)
# to (\Code\FINAL-PROJECT Main.r) including documentation and changing the print functions:
#		print_kable() -> print_table()
#		print_rules_kable() -> print_rules_table()

convert_to_main.r <- function() {
  
  rmd.file <- paste(report.dir, "FINAL-PROJECT.Rmd", sep="/")
  r.function.file <- paste(code.dir, "FINAL-PROJECT Main.r", sep="/")
