---
title: STAT5703 Data Mining I - Final Project Report - Appendix - Winter 2018
author: Michael Armanious (100978616), Alex El-Hajj (100887389), Muneer Khan (100650973)
  and Enrique Reveron (101066270)
date: "April 17, 2018"
output:
  word_document:
    toc: yes
...
number_sections: yes
classoption: a4paper
subtitle: \textbf{FINAL PROJECT ASSIGNMENT$:$ VISUALIZATION, DIMENSION REDUCTION,
  DATA REDUCTION, UNSUPERVISED AND SUPERVISED LEARNING}
...
The **folder** variables needs to be updated in order to reproduce the work.
...
## 1.9 Create New Dataset with Factor Variables and Names
...
We will split the dataset using 1/3 for test and 2/3 for training, for each Num type (0,1,2,3)
...
# 4. Logistic Regression (Quasibinomial) (glm)
...
Because the dataset don't have many variables (only 13) let's validate if make sense to do PCA for Kmeans:
...
Let's do PCA on the test set and create the cluster with the k and best_seed:
...
Let's check the K-means output
...
We can see the main results here: 
...
```{r,echo = FALSE, warning=FALSE,fig.height= 12, fig.width=6}
